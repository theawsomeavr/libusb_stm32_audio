...
static void evt_poll(usbd_device *dev, usbd_evt_callback callback) {
    ...
    while (1) {
        ...
        if (_t & USB_OTG_GINTSTS_USBRST) {
            ...
            for(int cur_ep = 0; cur_ep < MAX_EP; cur_ep++){
                USB_OTG_OUTEndpointTypeDef* epo = EPOUT(cur_ep);
                if (epo->DOEPINT & USB_OTG_DOEPINT_XFRC) {
                    epo->DOEPINT = USB_OTG_DOEPINT_XFRC;

                    uint8_t ep_is_isochr =
                        _FLD2VAL(USB_OTG_DOEPCTL_EPTYP, epo->DOEPCTL) == USB_EPTYPE_ISOCHRONUS;

                    // Since isochronous endpoints can only receive data on the
                    // specified odd or even frame num, after each reception we must
                    // configure the EONUM field
                    if (ep_is_isochr) {
                        uint8_t odd_now = _FLD2VAL(USB_OTG_DSTS_FNSOF, OTGD->DSTS) & 1u;
                        if (odd_now) {
                            epo->DOEPCTL |= USB_OTG_DOEPCTL_SD0PID_SEVNFRM;
                        } else {
                            epo->DOEPCTL |= USB_OTG_DOEPCTL_SODDFRM;
                        }
                    }
                }
            }
